...
class SimpleStorageService:
    ...
    def load_model(self, model_name: str, bucket_name: str, model_dir: str = None, disease_type: str = None) -> object:
        """
        Loads a serialized model from the specified S3 bucket.
        
        Args:
            model_name (str): Filename of the model (e.g. model.pkl)
            bucket_name (str): S3 bucket name
            model_dir (str): Optional parent directory in S3
            disease_type (str): Optional disease type subfolder (e.g. 'diabetes', 'heart')
        ...
        try:
            model_path_parts = []
            if model_dir:
                model_path_parts.append(model_dir)
            if disease_type:
                model_path_parts.append(disease_type)
            model_path_parts.append(model_name)

            model_key = "/".join(model_path_parts)

            file_object = self.get_file_object(model_key, bucket_name)
            model_obj = self.read_object(file_object, decode=False)
            model = pickle.loads(model_obj)
            logging.info(f"Model for {disease_type} loaded from S3: {model_key}")
            return model
        except Exception as e:
            raise MyException(e, sys) from e
